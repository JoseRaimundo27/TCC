...
\section{Discussão dos Resultados da Simulação}
...
\subsection{Discussão dos Resultados da Visualização Gráfica}

A integração entre o AnyLogic e o NVIDIA Omniverse permitiu ampliar a capacidade de visualização da simulação desenvolvida. Diferentemente da visualização tridimensional padrão do AnyLogic, o ambiente construído no NVIDIA Omniverse proporcionou maior realismo visual, permitindo representar elementos característicos do canteiro de obras, como vegetação, materiais espalhados, equipamentos e condições do terreno.

Os resultados demonstraram que a estilização gráfica contribuiu para uma melhor interpretação espacial das atividades executadas pelos trabalhadores durante o processo de montagem e desmontagem das fôrmas. A visualização em tempo real permitiu acompanhar deslocamentos, interações entre agentes e organização do ambiente de trabalho de forma mais intuitiva.

Além do aspecto visual, a integração também auxiliou na validação qualitativa do modelo de simulação. A reprodução do ambiente de obra com características semelhantes às observadas nas visitas técnicas permitiu verificar se os fluxos de movimentação, posicionamento de objetos e dinâmica operacional estavam coerentes com o cenário real analisado.

Entretanto, observou-se que a utilização do NVIDIA Omniverse aumenta o custo computacional da simulação, especialmente devido à renderização em tempo real dos elementos tridimensionais e efeitos de iluminação. Além disso, algumas adaptações visuais precisaram ser realizadas devido à indisponibilidade de determinados modelos específicos na biblioteca de \textit{assets}, como ocorreu na representação das fôrmas. Também foram observados alguns \textit{bugs} durante o processo de sincronização e renderização entre o AnyLogic e o NVIDIA Omniverse. Em determinados momentos, algumas fôrmas deixavam de ser corretamente renderizadas no ambiente virtual, ocasionando inconsistências visuais na simulação. Como consequência, alguns trabalhadores apareciam executando movimentações sem transportar as respectivas fôrmas, gerando divergências entre o estado lógico da simulação e sua representação gráfica no Omniverse.

Mesmo com essas limitações, os resultados indicam que a integração entre simulação híbrida e ambientes virtuais imersivos possui elevado potencial para aplicações na construção civil, principalmente em atividades relacionadas à análise operacional, treinamento, validação de processos e apoio à tomada de decisão.

\section{Discussão dos Resultados do Autômato Diagnosticador $G_d$}
...
Nesse sentido, o Evento relacionado à verificação das condições iniciais constitui um ponto crítico do processo sob a perspectiva de monitoramento e diagnóstico. Assim, torna-se recomendável que gestores adotem mecanismos adicionais de controle para essa etapa específica, como checklists digitais, formulários de inspeção, sensores de confirmação de atividade ou sistemas de validação operacional, visando aumentar a observabilidade do processo e reduzir a ocorrência de falhas não detectáveis no ambiente de obra.
