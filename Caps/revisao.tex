...
\begin{equation}
	G_d=
	(X_d,\ \Sigma_d,\ f_d,\ \Gamma_d,\ x_{0,d},\ X_{m,d})
	\label{eq:diagnosticador}
\end{equation}
...
\begin{enumerate}
	\item[(i)] Obtenha a composição paralela $G \parallel A_l$, sendo $A_l$ o autômato rotulador de dois Estados, projetado para alterar seu Estado de "Normal" para "Falha" irreversivelmente após a ocorrência de $\sigma_f$, conforme ilustrado na Figura \ref{fig:exemplo-rotulador};
...
Contudo, a simples observação física não é suficiente para capturar essas infrações, uma vez que elas ocorrem, na maioria das vezes, de forma silenciosa. O trabalhador burla os mecanismos de controle sem deixar rastros imediatos, o que faz com que esses erros se comportem, na ótica da simulação, como Eventos de Falha Não Observáveis.

É exatamente para estudar essa lacuna de rastreabilidade percebida pela Simulação Híbrida que surge a necessidade da aplicação da teoria de SED,. O estudo do SED é uma alternativa para este trabalho porque permite avançar da simples constatação do problema para o diagnóstico formal. O mapeamento das atividades realizadas ou não do canteiro servirá como base para a construção de um modelo capaz de rastrear o fluxo visível (Observável) da obra e inferir, matematicamente, a ocorrência dessas anomalias ocultas, conectando as decisões do trabalhador à detecção analítica de falhas no processo produtivo.
